class BaseElements {
    ...
    /**
     * Finds the element with the instance's locator
     * then invokes Selenium Webdriver sendKeys method.
     * It keeps sending keys until it is successful or times out, swallowing the rejected promise.
     * @param {string} keysToSend - any valid string or key object recognized by Selenium WebDriver
     * @param {number} [timeout=20000] - amount of time to pass into the selenium driver.wait function
     * @returns {Promise} void
     */
    sendKeysUntil(keysToSend, timeout = 20000) {
        return driver.wait(() => this.find()
            .then(element => element.sendKeys(keysToSend))
    .catch(() => {
            return false;
    })
    .then(() => true, () => false, timeout,
            `sendKeysUntil timed out after ${timeout} ms. ${this.locatorString}`)
    );
    }
